...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen bg-slate-100">
      ...
      {/* 전체 레이아웃 - 헤더 아래부터 3단 */}
      <div className="mx-auto max-w-5xl px-4 pb-8 flex gap-6 items-start">
        ...
        <div className="hidden lg:flex flex-col gap-4 w-64 shrink-0 sticky top-6">
          <div className="rounded-2xl bg-white p-5 shadow-sm border border-slate-200">
            ...
            <div className="space-y-3">
              {[
                { icon: "📍", title: "중간지점 자동 계산", desc: "부부 직장역 기준으로 최적 위치를 찾아요" },
                { icon: "📊", title: "실거래가 기반", desc: "국토부 실거래가 데이터로 정확해요" },
                { icon: "⭐", title: "AI 점수 추천", desc: "예산·출퇴근·학군 등 100점 만점 분석" },
                { icon: "🔨", title: "인테리어 비용 안내", desc: "연식별 예상 인테리어 비용 제공" },
              ].map((item, i) => (
